...
// 当前校验要求步骤与预期一一对应且均不少于 2 条（isValidGeneratedCaseRow）
...
TC001,登录,正常流程,输入正确用户名和密码登录成功,高,用户已注册,"1. 输入正确用户名\\n2. 输入正确密码\\n3. 点击登录","1. 用户名校验通过\\n2. 密码校验通过\\n3. 成功进入首页"`
...
TC099,登录,反向流程,密码错误,中,用户已注册,"1. 输入错误密码\\n2. 点击登录","1. 密码校验失败\\n2. 提示密码错误"
TC102,登录,边界校验,密码过短,中,打开登录页,"1. 输入短密码\\n2. 点击登录","1. 长度校验失败\\n2. 提示密码长度不足"`
