...
import { HTTP_METHODS, BODY_TYPES } from '../../data/http-request-types'
import type { HttpRequestData } from '../../lib/jmeter-types'
...
interface HttpRequestConfigProps {
  ...
  fieldErrors?: Partial<Record<'domain' | 'path', string>>
  onFieldChange?: (field: 'domain' | 'path') => void
}

const inputCls = 'field-control'
const labelCls = 'field-label text-xs'

export function HttpRequestConfig({ value, onChange, fieldErrors, onFieldChange }: HttpRequestConfigProps) {
        ...
        <div className="relative z-[1] grid grid-cols-[100px_70px_1fr_70px_1fr] items-end gap-2.5 max-lg:grid-cols-[90px_65px_1fr_65px_1fr] max-sm:grid-cols-2 max-sm:gap-3">
          ...
          <div>
            ...
            <input
              ...
              className={`${inputCls} ${fieldErrors?.domain ? 'field-control-error' : ''}`}
              aria-invalid={Boolean(fieldErrors?.domain)}
            />
            {fieldErrors?.domain && <p className="field-error">{fieldErrors.domain}</p>}
          </div>
          <div>
          ...
          <div>
            ...
            <input
              ...
              className={`${inputCls} ${fieldErrors?.path ? 'field-control-error' : ''}`}
              aria-invalid={Boolean(fieldErrors?.path)}
            />
            {fieldErrors?.path && <p className="field-error">{fieldErrors.path}</p>}
          </div>
        </div>
      </div>
