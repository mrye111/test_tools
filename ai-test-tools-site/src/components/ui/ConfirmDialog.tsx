...
export function ConfirmDialog({
  ...
  // 无需自行缓存文案快照：ModalShell 会在退出动画期间继续渲染 open 时的 children 快照，
  // 父组件在关闭的同时清空关联数据（如待删除对象）不影响退出动画展示。
  return (
    <ModalShell open={open} onClose={onCancel} closeOnBackdrop={false}>
        ...
        >
          <div className="p-6">
            <div className="flex items-start gap-4">
              <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border ${
                danger
                  ? 'border-[oklch(0.55_0.2_25/0.2)] bg-[oklch(0.55_0.2_25/0.1)] text-danger'
          ...
          <div className="flex flex-wrap justify-end gap-3 border-t border-white/60 bg-white/40 px-6 py-4">
            ...
            <button
              ...
              onClick={onConfirm}
              className={danger
                ? 'rounded-xl bg-danger px-4 py-2 text-sm font-semibold text-white shadow-[0_16px_36px_-18px_oklch(0.55_0.2_25/0.5)] transition-all duration-200 hover:-translate-y-0.5 hover:brightness-105 active:translate-y-0 active:scale-[0.98] focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus-ring)]'
