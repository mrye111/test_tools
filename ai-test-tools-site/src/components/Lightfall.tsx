...
export function Lightfall({
  ...
  useEffect(() => {
    ...
    // 无 WebGL 环境（老设备/测试环境）时优雅降级：只保留深色舞台背景
    ...
    // 监听挂在 window 上：内容层覆盖在画布上方，直接监听 canvas 会在
    // 指针穿过中央内容区时丢失事件，导致光照冻结-跳变（表现为"卡顿"）
    ...
    const loop = (t: number) => {
      ...
      if (!paused && !reduceMotion && inView && programRef.current && meshRef.current) {
        try {
