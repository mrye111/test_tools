  ...
  "issues": [
    {
      ...
      "description": "问题描述",
      "example": "一个具体例子说明问题在哪（≤60字，用具体数值或生活化场景，让非技术人员也能秒懂）",
...
2. **问题日志**（核心价值）：找出歧义（模糊措辞）、缺失（只写正常路径、缺错误处理/异常分支）、冲突（条目间矛盾）、不可测（无客观判定标准）四类问题。quote 必须是原文逐字引用，严禁编造。example 必须具体：比如区间问题就说「0~1000 次按 0.01 元，1000~5000 按 0.008 元——1000.5 次落在哪档？」，不要复述问题描述。没有问题的需求不要硬凑。
